import React, { useState } from "react";
// import { useDispatch } from "react-redux";
// import { login } from "../store/slice/IsLoggedInSlice";
import { Link } from "react-router-dom";
import axios from "axios";
import "./Login.css";

const Login = () => {
  // const dispatch = useDispatch();

  // const loginBtn = () => {
  //   dispatch(login());
  // };

  const [isLoading, setIsLoading] = useState(false);
  const [login, setLogin] = useState({
    email_address: "",
    password: "",
  });
  const [showPassword, setShowPassword] = useState(false);

  const onChange = (e) => {
    const { name, value } = e.target;
    setLogin({ ...login, [name]: value });
  };

  const onSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    try {
      const { data } = await axios.post("/api/login", login);
      console.log(data);
      setLogin({
        email_address: "",
        password: "",
      });
    } catch (error) {
      console.log(error);
    }
    setIsLoading(false);
  };

  return (
    <div className="login bg-[#909090] flex justify-center items-center container mx-auto sm:px-4 max-w-full p-6 h-screen">
      <div className="container mx-auto sm:px-0">
        <div className="flex flex-wrap ">
          <div className="sm:w-full pr-4 pl-4 md:w-1/2 flex items-center justify-center flex-col p-6 md:p-12">
            <form
              className="text-start flex flex-col w-full"
              onSubmit={onSubmit}
            >
              <div className="text-start w-full">
                <h3 className="flex items-center text-start mb-5 font-bold">
                  <svg
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    viewBox="0 0 24 24"
                    className="w-9 h-9 mr-2"
                  >
                    <path d="M15.182 15.182a4.5 4.5 0 01-6.364 0M21 12a9 9 0 11-18 0 9 9 0 0118 0zM9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75zm-.375 0h.008v.015h-.008V9.75zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75zm-.375 0h.008v.015h-.008V9.75z"></path>
                  </svg>
                  PyschHeal
                </h3>
              </div>
              <p className="font-medium mb-3 login-title welcome">
                Welcome Back!
              </p>
              <div className="flex flex-col mb-4">
                <label htmlFor="Email">Email Address</label>
                <input
                  type="email"
                  name="email_address"
                  onChange={onChange}
                  value={login.email_address}
                  className="border-2 border-gray-500 py-1.5 px-2 rounded-lg"
                  required
                />
              </div>
              <div className="flex flex-col relative font-medium">
                <label htmlFor="Password">Password</label>
                <input
                  type={showPassword === true ? "text" : "password"}
                  name="password"
                  onChange={onChange}
                  value={login.password}
                  className="border-2 border-gray-500 py-1.5 px-2 rounded-lg"
                  autoComplete="off"
                  required
                />
                {showPassword === true ? (
                  <svg
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    viewBox="0 0 24 24"
                    className="absolute right-4 top-[38px] h-5 w-5 text-gray-600 cursor-pointer"
                    onClick={() => setShowPassword(!showPassword)}
                  >
                    <path d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"></path>
                    <path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                  </svg>
                ) : (
                  <svg
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    viewBox="0 0 24 24"
                    className="absolute right-4 top-[38px] h-5 w-5 text-gray-600 cursor-pointer"
                    onClick={() => setShowPassword(!showPassword)}
                  >
                    <path d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88"></path>
                  </svg>
                )}
              </div>
              <div className="text-[#109ee6] font-medium text-sm">
                Password must include atleast one uppercase, lowecase, number
                and symbols like [@ ! $ ? % &]
              </div>
              <Link
                to="/reset_password"
                className="text-gray-500 font-medium mt-1 text-sm hover:text-gray-700"
              >
                Forgot password ?
              </Link>
              <button
                className="mt-5 flex justify-center items-center"
                type="submit"
              >
                {isLoading ? (
                  <svg
                    className="w-6 h-6 text-transparent animate-spin fill-white"
                    viewBox="0 0 100 101"
                    fill="none"
                  >
                    <path
                      d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                      fill="currentColor"
                    />
                    <path
                      d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                      fill="currentFill"
                    />
                  </svg>
                ) : (
                  <div className="text-white flex justify-center items-center">
                    Login
                    <svg
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
                      viewBox="0 0 24 24"
                      className="w-5 h-5 ml-0.5"
                    >
                      <path d="M4.5 12h15m0 0l-6.75-6.75M19.5 12l-6.75 6.75"></path>
                    </svg>
                  </div>
                )}
              </button>
            </form>
            <div className="text-sm font-medium mt-2 text-center">
              New to PyschHeal?
              <Link to="/register" className="text-[#109ee6] ml-1">
                Register Here
              </Link>{" "}
            </div>
          </div>
          <div className="vector-div hidden md:flex md:w-1/2 overflow-hidden">
            <img
              src="loginImg.jpg"
              alt="Be Happy"
              className="max-w-full h-auto"
            />
          </div>
        </div>
      </div>
    </div>
  );
};

export default Login;
