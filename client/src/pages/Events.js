...
const Events = () => {
  ...
  useEffect(() => {
    const NGO = NGOs.filter((ngo) => ngo._id === cookies.user_id);
    if (NGO.length === 0) {
      return;
    }
    setEvents(NGO[0].events || []);
  }, [NGOs]);
      ...
      <div className="tabs my-16">
        <ul className=" text-sm font-medium text-center text-gray-500 divide-x divide-gray-200 rounded-lg shadow sm:flex">
          <li className="w-full" onClick={() => changeTab(0)}>
            <a
              className={`inline-block w-full p-4 text-gray-700 rounded-l-lg focus:outline-none ${
          ...
          <li className="w-full" onClick={() => changeTab(1)}>
            <a
              href="#"
              className={`inline-block w-full p-4 text-gray-700 rounded-r-lg  focus:outline-none ${
